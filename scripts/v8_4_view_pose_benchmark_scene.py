...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use eval pose dump relative matrices for GT/corrected cameras, "
...
def load_pose_dump_external_raw0_cam_dicts(
    ...
    if gt_rel.shape[0] < num_frames or corrected_rel.shape[0] < num_frames:
        raise ValueError(
            f"Pose dump frame count is smaller than viewer frame count: "
            f"gt={gt_rel.shape[0]}, corrected={corrected_rel.shape[0]}, viewer={num_frames}"
        )
...
def main() -> None:
    ...
    if args.use_pose_dump_external_raw0:
        raw_cam_dict, corrected_cam_dict, gt_cam_dict = load_pose_dump_external_raw0_cam_dicts(
            record, args.eval_dir, raw_cam_dir, num_frames
        )
        print("Camera overlays use pose dump relative matrices anchored to gray raw frame 0.")

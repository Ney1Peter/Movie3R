...
# ============================================
# Movie3R V5.1-LAST2 4GPU 30 epoch training script
# 每卡 batch_size=24，全局 batch_size=96
# 当前配置要求 config/train.yaml 中 shot_pose_layers='last2'
# ============================================
...
# 30 epoch 观察完整收敛趋势，默认不让 early stopping 提前截停。
EARLY_STOPPING_PATIENCE=${EARLY_STOPPING_PATIENCE:-999999}
MASTER_PORT=${MASTER_PORT:-29516}
RUN_NAME=${RUN_NAME:-training-4gpu-bz24-30ep-shot-v5_1-last2-$(date +%Y%m%d-%H%M%S)}
...
echo "=========================================="
echo "Movie3R V5.1-LAST2 4GPU 30 epoch training"
echo "=========================================="
...
if "shot_pose_layers='last2'" not in cfg.model:
    raise RuntimeError("Expected config/train.yaml to use shot_pose_layers='last2'")

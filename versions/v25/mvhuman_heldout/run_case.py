#!/usr/bin/env python3
"""Run the frozen causal Bridge3R stack on one MVHuman MVH150 RGB case.

Only the prediction-only runtime row is visible to this process.  Camera IDs,
the true cut index, calibration, masks, angle strata, and 3-D labels remain in
the evaluator manifest and are never opened here.
"""
...
def main() -> None:
    ...
    if proposal is None:
        ...
        candidate, geometry, candidate_runtime = frozen.run_transaction(
            current_model, current_layer, topology, paths, int(proposal), device, args.size, "mvhuman_causal_first_positive"
        )
        methods = {
            "m0_strict_human3r": strict,
    ...
    if proposal is None:
        ...
        runtime["bridge3r"] = {"source": "learned coarse gauge at detector first positive", "association_pairs": pairs, "diagnostics": diagnostics}
    ...
    print(json.dumps({"case_id": record["case_id"], "cache": str(output), "detector_first_positive": proposal}, indent=2))

#!/usr/bin/env python3
"""Legacy V20 native-token probe retained inside Movie3R-Multi V13."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_dir", type=Path, default=ROOT / "output/v13/native_token"
    )
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or not str(args.device).startswith("cuda"):
        raise RuntimeError("Movie3R-Multi V13 token probe requires CUDA")
    ...
    report = {
        "experiment": "Movie3R-Multi V13 native-token feasibility probe",
        "legacy_experiment": "V20 native-token two-example feasibility probe",
        "scope": (
    ...
    report_path = args.output_dir / "v13_two_example_probe.json"
    ...
    torch.save(
        ...
        args.output_dir / "v13_two_example_token_dump.pt",
    )
    ...
    print(f">> V13 report: {report_path}", flush=True)

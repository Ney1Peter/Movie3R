...
V9.0 是此前已经完成训练的学习式版本，不是当前 V12 显式几何主线。它在原版
...
它不包含 V12 的 pre-decode hard reset、V16、DA3/Keypoint shared scale 或显式
Boundary，也不包含 V13 多人 consensus。
...
| 完整 resolved config | `versions/v9/resolved_config.yaml` |
...
```bash
PYTHONPATH=src:. .venv/bin/python versions/v9/run.py \
...
V9 可以用于研究 learned relation correction，但不应与 V12 的显式 hard-reset

# Movie3R-Single V12.0

## 定位

V12 是当前效果优先的单人 camera-cut 正式版。它冻结 Human3R，不训练新的 SE(3)
网络，而是在 cut 时对 fresh shot-local reconstruction 求一次显式 similarity。该实现
由历史 V14.7 系列实验完成冻结：
...
| 项目 | 值 |
|---|---|
| 正式名称 | Movie3R-Single V12.0 |
| 功能 commit | `af92478` |
| Git tag | `movie3r-v12-single` |
...
`af92478` 在历史 V14.7 冻结基础上加入了任意多 cut 的实际 demo 和 viewer，不改变
原算法或 180-cut 数值。V14.x 在本目录中仅表示历史实验，不是另一条正式版本线。
...
```bash
PYTHONPATH=src:. .venv/bin/python versions/v12/run.py \
  --seq_path /path/to/images \
  --cuts 5 10 15 \
  --output_dir output/v12/example \
  --device cuda:0
```
...
```bash
PYTHONPATH=src:. .venv/bin/python versions/v12/viewer.py \
  --result_dir output/v12/example \
  --port 8080
```
...
- `versions/v12/experiments/v14_5_true_recurrent_multicut_audit.py`
- `versions/v12/experiments/v14_5_multicut_interactive_viewer.py`

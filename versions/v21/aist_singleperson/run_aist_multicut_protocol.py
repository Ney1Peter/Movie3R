#!/usr/bin/env python3
"""Resumable multi-GPU protocol driver for AIST++ MC150-3/MC150-4.

The coordinator alone can join each RGB runtime row to its evaluator row;
the CUDA subprocess is passed the RGB runtime manifest and an integer line
only.  This retains the separation used by CS150 while making it impossible
to accidentally call the single-cut runner/evaluator for multi-cut results.
"""
...
SCHEMA = "Bridge3R-AIST-SinglePerson-MC150-run-protocol-v1"
RUNNER = Path(__file__).with_name("run_aist_multicut_case.py")
EVALUATOR = Path(__file__).with_name("evaluate_aist_multicut.py")
...
def validate_pair(args: argparse.Namespace) -> list[tuple[int, dict[str, Any], dict[str, Any]]]:
    ...
    for line, runtime in enumerate(runtime_rows, start=1):
        ...
        if runtime["dataset"] != "AIST++" or runtime["protocol"] != args.protocol or runtime["role"] != args.role:
        ...
        if evaluator.get("role") != args.role or evaluator.get("protocol") != args.protocol:
...
def main() -> None:
    ...
    payload = {
        ...
        "runtime_evaluator_separation": "CUDA runner received runtime rows only; evaluator rows were consumed only after cache creation.",
    }
    ...
    print(json.dumps({"summary": str(summary), "protocol": args.protocol, "selected": len(items), "ok": len(results) - error_count, "errors": error_count}, ensure_ascii=False, indent=2))
    if error_count:
        raise SystemExit(1)

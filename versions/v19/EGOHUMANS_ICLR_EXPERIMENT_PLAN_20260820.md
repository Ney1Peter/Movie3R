...
## 13. 零人体帧的实现修复（2026-08-20）

holdout 推理首次遇到 Human3R 在单帧未检出人体时省略全部 `smpl_*` 字段。旧解码器把它误当作必须存在的键并使整个 case 进程崩溃；修正后将这种模型输出忠实解码为“该帧 0 个预测人体”，所有显式分支以空 mesh batch 安全透传，并由统一 evaluator 的 coverage、identity 和误差规则处理。该修复不访问 GT、不改变预测、不增加候选，也不将难例从协议中删除。
